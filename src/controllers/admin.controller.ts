  ...
  try {
    if (!req.session?.member) {
      ...
      const notifications = {
          ...
          total: memberStats.pending + memberStats.newUsers + eventStats.newEvents + boardStats.newBoards
      };
...
adminController.processSignup = async (req: AdminRequest, res: Response) => {
  try {
    ...
    res.send(`<script> alert("Signup Successful!"); window.location.replace('/admin/login')</script>`);
  } catch (err: any) {
